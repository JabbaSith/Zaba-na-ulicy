...
TEST(SmallCarTest, MovesForwardRight) {
    SmallCar c(0, 100.0f, CAR_SPEED);
    float before = c.getX();
    c.move();
    EXPECT_FLOAT_EQ(c.getX(), before + CAR_SPEED);
}
...
TEST(TruckTest, SameSpeedAsOtherCars) {
    // Wszystkie typy mają teraz CAR_SPEED – brak różnic prędkości
    SmallCar sc(0, 0.0f, CAR_SPEED);
    Truck    tr(0, 0.0f, CAR_SPEED);
    float xSC = sc.getX(); sc.move();
    float xTr = tr.getX(); tr.move();
    EXPECT_FLOAT_EQ(sc.getX() - xSC, tr.getX() - xTr);
}
