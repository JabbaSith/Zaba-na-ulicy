 ...
 * @file lane.cpp
 *
 * Spawn rate oparty na minimalnej odległości (MIN_GAP) między autami –
 * nowe auto pojawi się tylko gdy ostatnie odjechało wystarczająco daleko.
 * Kierunek: parzyste pasy (0,2,4) jadą w PRAWO (+X),
 *           nieparzyste  (1,3,5) jadą w LEWO  (-X).
 */
...
/// Prędkość bazowa pojazdu danego typu.
static float carSpeed(int type) {
    if (type == 0) return SPEED_SMALL;
    if (type == 1) return SPEED_TRUCK;
    return SPEED_SUV;
}
...
bool Lane::canSpawn() const {
    ...
    if (goRight) {
        // Auta wjeżdżają z lewej (x=-width), więc sprawdzamy ostatnie po X
        // Szukamy tego o najmniejszym X (świeżo spawnowanego)
        float minX = m_cars[0]->getX();
        for (auto &c : m_cars) minX = std::min(minX, c->getX());
        // Musi odjechać MIN_GAP od krawędzi lewej (x=0)
        return minX > (float)MIN_GAP;
    } else {
        // Auta wjeżdżają z prawej, szukamy max X
...
void Lane::spawnCar() {
    ...
    int type = std::rand() % 3;
    float baseSpeed = carSpeed(type);
    float speed = goRight ? baseSpeed : -baseSpeed;
