    ...
    , m_tick(0)
{
    ...
    connect(&m_renderTimer, &QTimer::timeout, this, &Game::renderTick);
...
void Game::startGame() {
    resetGame();
    m_running = true;
    m_timer.start(GAME_TICK_MS);        // decyzje/input – może być nadpisany przez setTickInterval
    m_renderTimer.start(GAME_TICK_MS);  // ruch aut + render – zawsze 30ms
}

void Game::pauseGame() {
    m_running ? m_timer.stop()  : m_timer.start(m_timer.interval());
    m_running ? m_renderTimer.stop() : m_renderTimer.start(GAME_TICK_MS);
    m_running = !m_running;
}
...
void Game::handleInput(Direction dir) {
    ...
    // Oblicz wynik na podstawie aktualnej pozycji żaby (im wyżej, tym więcej pkt).
    // START_Y (≈600) → 0 pkt,  Y=0 (META) → 100 pkt.
    int posScore = std::clamp(
        static_cast<int>((1.0f - static_cast<float>(m_player->getY()) / START_Y) * 100),
        0, 100);

    // Wynik bieżący = maximum z dotychczasowego (wynik nie maleje w trakcie rundy).
    int newScore = std::max(m_score->getCurrentScore(), posScore);
    if (newScore != m_score->getCurrentScore()) {
        // Przelicz przez addPoints – to emituje scoreChanged i aktualizuje rekord.
        m_score->addPoints(newScore - m_score->getCurrentScore());
    }

    // Natychmiastowe uaktualnienie rekordu (bez zwiększania currentScore)
    // – np. gdy gracz doszedł do 80pkt i zginie, rekord musi zostać 80.
    m_score->updateHighScore(newScore);

    if (m_player->getY() <= GOAL_Y) {
        // Dotarcie do mety = pełne 100 pkt.
        m_score->addPoints(100 - m_score->getCurrentScore());
        emit goalReached();
    }
}
...
void Game::setTickInterval(int ms) {
    m_timer.setInterval(ms);
}

void Game::warmup(int ticks) {
    // Symuluj ruch aut synchronicznie – bez emitowania sygnałów, bez żaby.
    for (int i = 0; i < ticks; ++i)
        for (auto &lane : m_lanes)
            lane->update();
    // Odśwież scenę raz po rozgrzewce.
    renderFrame();
}
...
const std::vector<std::unique_ptr<Lane>>& Game::getLanes() const { return m_lanes; }

// ── Tick ──────────────────────────────────────────────────────────────────────
void Game::tick() {
    // Emitowany co AI_TICK_MS (400ms w trybie AI, 30ms w trybie gracza).
    // W trybie AI: GameWindow::onAiTick() odbiera ten sygnał i wykonuje ruch żaby.
    // W trybie gracza: ten slot nie jest podpięty do niczego (input idzie przez keyPressEvent).
    emit tickReady();
}

void Game::renderTick() {
    // Wywoływany zawsze co GAME_TICK_MS (30ms) niezależnie od trybu.
    if (!m_running) return;
    ++m_tick;
